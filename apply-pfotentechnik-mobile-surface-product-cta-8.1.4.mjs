...
const PATCH_ID = "pfotentechnik-mobile-surface-product-cta-8.1.4";
...
const PREMIUM_MARKER =
  "/* === PfotenTechnik Mobile Surface Product CTA 8.1.4 === */";
...
/* === End PfotenTechnik Mobile Surface Product CTA 8.1.4 === */
`,
    eol
  );
...
const secondaryCtaLabel =
  ...
  if (/\bconst testStatus\s*=/.test(next)) {
    ...
    if (/\bconst testStatus\s*=/.test(next)) {
      fail("ProductRenderer: Statusvariable konnte nicht entfernt werden.");
    }

    changes.push("Nicht mehr benötigte Statusvariable entfernt");
  } else {
    changes.push("Statusvariable war bereits entfernt");
  }
  ...
  if (/\{testStatus\s*&&\s*<div><dt>Status<\/dt>/.test(next)) {
    ...
    if (/\{testStatus\s*&&\s*<div><dt>Status<\/dt>/.test(next)) {
      fail("ProductRenderer: Statuszeile konnte nicht entfernt werden.");
    }

    changes.push("Statuszeile aus der Buybox entfernt");
  } else {
    changes.push("Statuszeile war bereits entfernt");
  }
