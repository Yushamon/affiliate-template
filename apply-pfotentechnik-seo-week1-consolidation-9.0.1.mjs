...
const PATCH_ID = "pfotentechnik-seo-week1-consolidation-9.0.1";
...
function findStatementEnd(source, startIndex) {
  ...
  if (end < 0) fail("Anweisungsende in buildComparisonViewModel.ts nicht gefunden.");
  return end + 1;
}
...
function patchComparisonViewModel(content) {
  ...
  const explicitItemsBlock = `  const explicitItems = data.items.filter(\n    (item, index, source) =>\n      source.findIndex(\n        (candidate) =>\n          candidate.type === item.type &&\n          candidate.slug === item.slug\n      ) === index\n  );`;

  if (!/\bconst\s+explicitItems\s*=\s*data\.items\.filter\s*\(/m.test(next)) {
    const explicitSlugsMatch = /\n\s{2}const\s+explicitSlugs\s*=\s*new\s+Set\s*\(/m.exec(next);
    const automaticMatch = /\n\s{2}const\s+automaticItems\s*=/m.exec(next);
    const itemsMatch = /\n\s{2}const\s+items\s*=/m.exec(next);
    const recommendationMatch = /\n\s{2}const\s+automaticRecommendation\s*=/m.exec(next);

    if (explicitSlugsMatch) {
      const blockStart = explicitSlugsMatch.index + 1;
      const blockEnd = findStatementEnd(next, blockStart);
      const oldBlock = next.slice(blockStart, blockEnd);
      next = `${next.slice(0, blockStart)}${explicitItemsBlock}\n\n${oldBlock}${next.slice(blockEnd)}`;
    } else {
      const insertionMatch = automaticMatch ?? itemsMatch ?? recommendationMatch;
      if (!insertionMatch) {
        fail("Kein semantischer Einfügepunkt vor der Comparison-Produktliste gefunden.");
      }
      const insertionIndex = insertionMatch.index + 1;
      next = `${next.slice(0, insertionIndex)}${explicitItemsBlock}\n\n${next.slice(insertionIndex)}`;
    }
  }

  const canonicalAutomaticBlock = `  // Kuratierte Vergleichslisten sind autoritativ. Automatische\n  // Produktzuordnung dient nur als Fallback, wenn keine items gepflegt sind.\n  const automaticItems = explicitItems.length === 0\n    ? products\n      .filter((product) =>\n        product.data.comparisons.includes(data.slug)\n      )\n      .map((product) => ({\n        slug: product.data.slug,\n        label: product.data.title,\n        type: "product" as const,\n        recommendation: product.data.recommendation,\n        values: {}\n      }))\n    : [];`;

  const alreadyCanonical =
    next.includes("const automaticItems = explicitItems.length === 0") &&
    next.includes("const items = [...explicitItems, ...automaticItems];");

  if (alreadyCanonical) {
    const explicitCount = (next.match(/\bconst\s+explicitItems\s*=/g) ?? []).length;
    const automaticCount = (next.match(/\bconst\s+automaticItems\s*=/g) ?? []).length;
    const itemsCount = (next.match(/\bconst\s+items\s*=/g) ?? []).length;
    if (explicitCount !== 1 || automaticCount !== 1 || itemsCount !== 1) {
      fail(`Comparison-Pipeline ist nicht eindeutig: explicitItems=${explicitCount}, automaticItems=${automaticCount}, items=${itemsCount}`);
    }
    return next;
  }
  ...
  const automaticDecl = /\n\s{2}const\s+automaticItems\s*=/m.exec(next);
  const itemsDecl = /\n\s{2}const\s+items\s*=/m.exec(next);
  const recommendationDecl = /\n\s{2}const\s+automaticRecommendation\s*=/m.exec(next);

  if (automaticDecl) {
    const blockStart = automaticDecl.index + 1;
    const following = [itemsDecl, recommendationDecl]
      .filter((match) => match && match.index > automaticDecl.index)
      .sort((a, b) => a.index - b.index)[0];
    const blockEnd = following
      ? following.index + 1
      : findStatementEnd(next, blockStart);
    next = `${next.slice(0, blockStart)}${canonicalAutomaticBlock}\n\n${next.slice(blockEnd)}`;
  } else {
    const insertion = itemsDecl ?? recommendationDecl;
    if (!insertion) {
      fail("Weder automaticItems noch ein nachfolgender Einfügepunkt gefunden.");
    }
    const insertionIndex = insertion.index + 1;
    next = `${next.slice(0, insertionIndex)}${canonicalAutomaticBlock}\n\n${next.slice(insertionIndex)}`;
  }

  const canonicalItems = `  const items = [...explicitItems, ...automaticItems];`;
  const currentItemsDecl = /\n\s{2}const\s+items\s*=/m.exec(next);
  const currentRecommendationDecl = /\n\s{2}const\s+automaticRecommendation\s*=/m.exec(next);

  if (currentItemsDecl) {
    const itemStart = currentItemsDecl.index + 1;
    const itemEnd = findStatementEnd(next, itemStart);
    next = `${next.slice(0, itemStart)}${canonicalItems}${next.slice(itemEnd)}`;
  } else if (currentRecommendationDecl) {
    const insertionIndex = currentRecommendationDecl.index + 1;
    next = `${next.slice(0, insertionIndex)}${canonicalItems}\n\n${next.slice(insertionIndex)}`;
  } else {
    fail("Anker für die finale items-Liste nicht gefunden.");
  }
  ...
  if (explicitCount !== 1 || automaticCount !== 1 || itemsCount !== 1) {
    fail(`Comparison-Pipeline nach Umbau nicht eindeutig: explicitItems=${explicitCount}, automaticItems=${automaticCount}, items=${itemsCount}`);
  }

  if (!next.includes("const automaticItems = explicitItems.length === 0")) {
    fail("Kuratierte Comparison-Items konnten nicht autoritativ gesetzt werden.");
  }
