...
const PATCH = "pfotentechnik-product-metadata-4.0.2";
...
const reportFile = path.join(reportsDir, "comparison-audit.json");
const outputReport = path.join(app, "reports", "product-metadata-4.0.2-report.json");
...
function buildManufacturerCatalog() {
  return markdownFiles(manufacturersDir).map((file) => {
    ...
    return { file, slug, key, name, aliases, tokenSet: new Set(tokens([slug, key, name, fileSlug].join(" "))) };
  });
}
  ...
  for (const value of currentValues.filter(Boolean)) {
    ...
    for (const alias of manufacturer.aliases) {
      if (n && alias && (n.includes(alias) || alias.includes(n))) {
        const shorter = Math.min(n.length, alias.length);
        if (shorter >= 4) {
          score += 45;
...
function brokenProductFiles() {
  run("npm", ["--workspace", "apps/pfotentechnik", "run", "comparison:audit"], false);

  if (!fs.existsSync(reportFile)) {
    fail("comparison-audit.json wurde nicht erzeugt.");
  }
  ...
  for (const issue of broken) {
    const normalized = String(issue.file || "").replace(/\\/g, "/");
    if (!normalized) continue;

    const absolute = normalized.startsWith("apps/")
      ? path.join(root, normalized)
      : path.join(app, normalized);

    if (fs.existsSync(absolute)) paths.add(path.resolve(absolute));
  }

  return { report, broken, files: [...paths] };
}
...
try {
  ...
  const { broken, files } = brokenProductFiles();

  console.log(`\n[${PATCH}] Defekte Herstellerreferenzen im Audit: ${broken.length}`);
  ...
  if (!files.length) {
    fail("Die betroffenen Produktdateien konnten aus dem Audit nicht aufgelöst werden.");
  }
  ...
  for (const file of files) {
    ...
    const before = JSON.stringify(product.data.manufacturer);
    const after = JSON.stringify(nextManufacturer);
  ...
  if (unresolved.length) {
    fail(`${unresolved.length} Herstellerreferenz(en) konnten nicht eindeutig repariert werden. Details: ${path.relative(root, outputReport)}`);
  }
  ...
  const finalReport = JSON.parse(read(reportFile));
  const remaining = finalReport.issues.filter((issue) =>
    issue.code === "MANUFACTURER_REFERENCE_BROKEN"
  );

  if (remaining.length) {
    fail(`Nach der Reparatur bestehen noch ${remaining.length} defekte Herstellerreferenzen.`);
  }

  console.log(`\n[${PATCH}] Fertig.`);
  console.log(`Herstellerreferenzen repariert: ${changes.length}`);
  console.log(`Comparison Audit: ${finalReport.summary.errors} Fehler, ${finalReport.summary.warnings} Warnungen`);
