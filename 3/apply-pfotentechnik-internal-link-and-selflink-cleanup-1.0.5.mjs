#!/usr/bin/env node
/**
 * PfotenTechnik Internal Link + Selflink Cleanup 1.0.5
 *
 * Ausführung im Root von Yushamon/affiliate-template:
 *   node apply-pfotentechnik-internal-link-and-selflink-cleanup-1.0.5.mjs
 *
 * Optionen:
...
function patchAdvisorSelfLinkPage() {
  ...
  // Ältere Varianten enthielten auf der Zielseite selbst noch eine AdvisorCta.
  ...
  // Der aktuell verbliebene Selflink stammt vom letzten Breadcrumb. Der aktuelle
  // Seitenpunkt bleibt sichtbar, erhält aber bewusst kein href.
  const linkedCurrentCrumb = /\{\s*label\s*:\s*["']Kaufberatung["']\s*,\s*href\s*:\s*["']\/berater\/futterautomat\/["']\s*\}/g;
  const breadcrumbMatches = [...after.matchAll(linkedCurrentCrumb)];
  if (breadcrumbMatches.length) {
    after = after.replace(linkedCurrentCrumb, '{label:"Kaufberatung"}');
    cleanupFindings.push({ type: "self-link-breadcrumb-neutralized", file: rel(file), target: "/berater/futterautomat/", occurrences: breadcrumbMatches.length });
  }

  if (after === before && before.includes('href:"/berater/futterautomat/"')) {
    fail("Der Berater-Selflink ist weiterhin vorhanden, konnte aber nicht sicher dem aktuellen Breadcrumb zugeordnet werden.");
  }

  writeIfChanged(file, after, "aktuellen Berater-Breadcrumb ohne Selflink ausgegeben");
}
