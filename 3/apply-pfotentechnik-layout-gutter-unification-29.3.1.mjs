...
const PATCH = "pfotentechnik-layout-gutter-unification-29.3.1";
...
const files = {
  ...
  test: path.join(root, "apps/pfotentechnik/test/layout-gutter-unification-29.3.1.test.mjs"),
};
...
// Alte lokale Gutter-Ownership vollständig entfernen, nicht nur den ersten Block.
comparisonCss = comparisonCss
  .replace(/^\s*--comparison-page-gutter\s*:[^;]+;\s*$/gm, "")
  .replace(/var\(\s*--comparison-page-gutter(?:\s*,[^)]*)?\)/g, "var(--pt-page-gutter, 12px)")
  .replace(/--comparison-page-gutter/g, "--pt-page-gutter");
comparisonCss = normalizeBlankLines(comparisonCss);
