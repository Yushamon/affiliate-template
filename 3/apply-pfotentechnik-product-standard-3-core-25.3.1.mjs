...
const NAME = "pfotentechnik-product-standard-3-core-25.3.1";
...
for (const target of targets) {
  ...
  } else if (source.includes("<ProductCategoryFitAssistant")) {
    console.log("[" + NAME + "] Bereits korrigiert: " + path.relative(ROOT, target));
  } else {
    throw new Error("Erwarteter veralteter Testanker nicht gefunden in " + path.relative(ROOT, target));
  }
}
