...
const NAME = "pfotentechnik-css-product-system-cleanup-22.8.0";
...
const REPORT_JSON = path.join(REPORT_DIR, "css-product-system-22.8.0.json");
const REPORT_MD = path.join(REPORT_DIR, "css-product-system-22.8.0.md");
...
test("Product-Box-Selektoren wurden aus product.css entfernt", () => {
  const product = fs.readFileSync(productFile, "utf8");
  assert.ok(!product.includes(".product-box-v2"));
  assert.ok(!product.includes(".product-box-image"));
  assert.ok(!product.includes(".product-box-specs"));
});
