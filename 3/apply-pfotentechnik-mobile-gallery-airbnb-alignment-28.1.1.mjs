...
const PATCH = "pfotentechnik-mobile-gallery-airbnb-alignment-28.1.1";
...
function replaceRule(css, selector, body) {
  ...
  while (pos < css.length) {
    const index = css.indexOf(selector, pos);
    if (index < 0) break;
    ...
    if (validBefore && validAfter && css[brace] === "{") {
      ...
      const match = { i: index, e: end + 1 };

      if (cssDepthAt(css, index) === 0) {
        rootMatches.push(match);
      } else {
        nestedMatches.push(match);
      }
    ...
    pos = index + selector.length;
  }

  if (rootMatches.length !== 1) {
    throw new Error(
      `CSS-Basisregel ${selector}: ${rootMatches.length} Root-Treffer, ` +
      `${nestedMatches.length} verschachtelte Treffer.`,
    );
  }
...
const T=path.join(APP,"test/mobile-gallery-airbnb-alignment-28.1.1.test.mjs");
