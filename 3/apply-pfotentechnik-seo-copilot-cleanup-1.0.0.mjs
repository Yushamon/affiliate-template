...
const validations = [
  ...
  {
    name: "SEO Release ohne Build",
    args: ["--workspace", "apps/pfotentechnik", "run", "seo:release:check:no-build"],
  },
  {
    name: "Astro Build",
...
const runValidations = (repoRoot) => {
  ...
  for (const validation of validations) {
    ...
    console.log(output);
    console.log(`[pfotentechnik-seo-copilot-cleanup-${VERSION}] ${passed ? "BESTANDEN" : "FEHLER"}: ${validation.name}`);
