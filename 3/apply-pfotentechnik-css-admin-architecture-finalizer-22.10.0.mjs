...
const NAME = "pfotentechnik-css-admin-architecture-finalizer-22.10.0";
...
/* 1. Operations extrahieren, sofern noch nicht geschehen. */
...
/* 2. Responsive-Regeln extrahieren, sofern noch nicht geschehen. */
...
const ownership = {
  ...
  layers: {
    foundation: {
      ...
      owns: [".seo-shell", ".seo-page-header", ".seo-title", ".seo-subtitle"]
    },
    panels: {
...
test("Statische Ownership-Marker leaken nicht in andere statische Layer", () => {
  const responsive = new Set(["responsive"]);
  for (const [owner, layer] of Object.entries(manifest.layers)) {
    if (responsive.has(owner)) continue;
    for (const marker of layer.owns) {
...
const tests = {
  ...
  "css-admin-foundation.test.mjs": focusedTest(
    "foundation",
    [".seo-shell", ".seo-page-header", ".seo-title"],
...
const score = 100
  - (remainingEntry.length ? 25 : 0)
  - Object.values(metrics).filter((item) => item.mediaQueries > 0).length * 0
  - Object.values(metrics).reduce((sum, item) => sum + item.importantDeclarations, 0) * 0;

const reportMd = `# SEO Admin CSS Architecture 22.10.0

## Ergebnis

- Status: finalisiert
- Architektur-Score: ${score}/100
...
- Der Entrypoint besitzt ausschließlich geordnete Imports.
- Jeder statische Selektor besitzt genau einen statischen Owner.
- Responsive Overrides dürfen Selektoren der statischen Layer wiederverwenden.
- Media Queries sind ausschließlich im Responsive-Layer erlaubt.
- Kein Layer importiert einen anderen Admin-Layer.
- Dark-Mode-Systemfallback gehört zum Responsive-Layer.
`;

const packageJson = JSON.parse(fs.readFileSync(PACKAGE, "utf8"));
packageJson.scripts ||= {};
packageJson.scripts["test:css-admin-architecture"] = "node --test test/css-admin-architecture.test.mjs";
packageJson.scripts["test:css-admin-foundation"] = "node --test test/css-admin-foundation.test.mjs";
packageJson.scripts["test:css-admin-panels"] = "node --test test/css-admin-panels.test.mjs";
...
packageJson.scripts["test:css-admin-responsive"] = "node --test test/css-admin-responsive.test.mjs";
