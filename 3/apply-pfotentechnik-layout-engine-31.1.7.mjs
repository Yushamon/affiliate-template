...
const PATCH = "pfotentechnik-layout-engine-31.1.7";
...
const files = {
  ...
  test: path.join(root, "apps/pfotentechnik/test/pfotentechnik-layout-engine-31.1.7.test.mjs")
};
...
const canonicalCss = `
/* Layout Engine 31.1.7: current rendered comparison cover */
.comparison-cover {
  display: grid;
...
function migrateCss(source) {
  ...
  next = removeBlocks(next, (selector) => {
    ...
    return isLegacyHeroSelector(normalized) ||
...
function migrateHero(source) {
  let next = source
    .replace(/data-comparison-cover=(?:"[^"]*"|'[^']*')/, 'data-comparison-cover="31.1.7"')
    .replace(/<header\s+class=(["'])comparison-cover\1/, '<header class="comparison-cover" data-layout-engine="31.1.7"');
  if (!/data-layout-engine="31\.1\.7"/.test(next)) {
    next = next.replace(
      /<header\s+class=(["'])comparison-cover\1/,
      '<header class="comparison-cover" data-layout-engine="31.1.7"'
    );
  }
  next = next.replace(/\s+data-layout-engine="31\.1\.7"\s+data-layout-engine="31\.1\.7"/g, ' data-layout-engine="31.1.7"');
  return next;
}
...
try {
  ...
  if (fs.existsSync(dist)) {
    ...
    if (!html.includes('data-layout-engine="31.1.7"')) {
