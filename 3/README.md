# PfotenTechnik Comparison Rendered UI Fix 15.4.1
...
```bash
node 3/apply-pfotentechnik-comparison-rendered-ui-fix-15.4.1.mjs
```

Optional ohne Prüfungen:

```bash
node 3/apply-pfotentechnik-comparison-rendered-ui-fix-15.4.1.mjs --skip-checks
```


## Korrektur gegenüber 15.4.0

Der Komponenten-Audit verlangt bei statischen Buttons die Klasse `pt-button`. 15.4.1 erfüllt diese Regel wieder. Die fehlerhaften visuellen Nebeneffekte werden nicht durch Entfernen der Primitive gelöst, sondern durch eine eng begrenzte Neutralisierung der `::before`- und `::after`-Dekorationen am Navigationstoggle.
