...
const commit = (message, paths) => {
  ...
  if (!staged) fail(`Commit „${message}“ enthält keine Änderungen.`);
...
function isKnownSafePartialInstallation(statusOutput) {
  ...
  const allowed = new Set(phaseOnePayloadFiles);
  if (!changed.every((relative) => allowed.has(relative))) return false;
...
function preflight() {
  ...
  if (dirty) {
    console.log("[product-operations-1.0.1] Sichere Teilinstallation aus Version 1.0.0 erkannt. Installation wird fortgesetzt.");
  }
}
