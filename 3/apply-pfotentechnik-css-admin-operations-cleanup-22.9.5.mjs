...
const NAME = "pfotentechnik-css-admin-operations-cleanup-22.9.5";
...
const REPORT_JSON = path.join(REPORT_DIR, "css-admin-operations-22.9.5.json");
const REPORT_MD = path.join(REPORT_DIR, "css-admin-operations-22.9.5.md");
...
test("Operations-Regeln wurden aus seo-admin.css entfernt", () => {
  const source = fs.readFileSync(sourceFile, "utf8");
  for (const forbidden of [
    ".seo-finding-list",
    ".seo-finding {",
    ...
    assert.ok(!source.includes(forbidden), "Noch im Hauptlayer: " + forbidden);
  }
});
...
const report = {
  ...
  migratedSystems: [
    "finding list",
    "finding cards",
    "blocker and regression states",
    "finding details and code blocks",
    "workspace summary",
    "workspace fact grid"
  ],
  retainedSystems: [
    "responsive grid adjustments",
    "mobile shell and header rules",
    "mobile actions",
    "workspace responsive collapse",
...
Nach diesem Schritt enthält \`seo-admin.css\` neben den Imports nur noch die
Responsive-Regeln und den automatischen Dark-Mode-Fallback.
`;
