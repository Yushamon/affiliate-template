...
const PATCH="pfotentechnik-cornerstone-performance-closure-33.2.1";
...
const testFile=path.join(app,"test","cornerstone-performance-closure-33.2.1.test.mjs");
...
`;
  ...
  ]) if(!src.includes(s)) throw new Error(`Redaktioneller Inhalt fehlt: ${s}`);

  for(const key of ["petlibro-granary-wifi-feeder","petlibro-granary-camera-feeder","petkit-yumshare-dual-hopper","petkit-fresh-element-solo","xiaomi-smart-pet-food-feeder-2","cat-mate-c500"])
...
test("redundante Premium-Blöcke entfernt",()=>{
  assert.doesNotMatch(content,/Ausgewählte smarte Futterautomaten/);
  assert.doesNotMatch(content,/Die wichtigsten Qualitätsmerkmale/);
});
test("Entscheidungsblöcke bleiben",()=>{
  for(const type of ["answer","quickFacts","scenarios","decision","mistakes"])
    assert.match(content,new RegExp('type: "'+type+'"'));
});
test("Vergleich und Hauptinhalt bleiben",()=>{
