...
const NAME = "pfotentechnik-anchor-governance-and-release-gate-1.0.1";
...
const current = read(AUDIT);
if (
  !current.includes('Release Build Output:') ||
  !current.includes('CANONICAL_COUNT_INVALID') ||
  !current.includes('SITEMAP_BUILD_TARGET_MISSING')
) {
  fail("Der bestehende Build-Output-Audit entspricht nicht der erwarteten Version 1.0.0.");
}
...
if (fs.existsSync(REPORT)) {
  try {
    ...
    log("Bisherige Befunde:");
    for (const [code, count] of [...counts].sort((a, b) => b[1] - a[1])) {
      log(`- ${code}: ${count}`);
    }
  } catch (error) {
    log(`Vorhandener Report konnte nicht ausgewertet werden: ${error.message}`);
  }
}
...
backup(AUDIT);
fs.writeFileSync(AUDIT, audit, "utf8");
...
if (auditResult.status !== 0) {
  if (fs.existsSync(REPORT)) {
    ...
    if (errors.length > 40) {
      console.error(`- … und ${errors.length - 40} weitere. Vollständig im Report.`);
    }
  }
  fail(
    "Der korrigierte Audit hat weiterhin echte Befunde erkannt. " +
    `Details: ${rel(REPORT)}`
  );
}
...
log("");
log("ABGESCHLOSSEN.");
log("Build-Output-Audit auf Version 1.0.1 aktualisiert.");
log("Indexierbarkeit, Admin-/API-/Fehlerseiten und Noindex werden differenziert behandelt.");
log("Sitemap-Routen werden gegen das tatsächliche dist-Routeninventar geprüft.");
log("Canonical-Duplikate werden dedupliziert, echte Konflikte bleiben harte Fehler.");
log("JSON-LD, main-Struktur, Redirect-Aliasse und fehlende Build-Ziele bleiben release-blockierend.");
