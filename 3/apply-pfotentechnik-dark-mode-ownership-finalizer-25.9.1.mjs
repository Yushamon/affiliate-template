...
const NAME = "pfotentechnik-dark-mode-ownership-finalizer-25.9.1";
...
const files = {
  ...
  test: path.join(APP, "test", "dark-mode-ownership-finalizer-25.9.1.test.mjs")
};
...
function removeMediaBlocksByPattern(source, headerPattern, shouldRemove) {
  let output = source;
  let searchFrom = 0;
  ...
  while (true) {
    ...
    const opening = output.indexOf("{", start);
    if (opening < 0) {
      throw new Error("CSS-Media-Block ohne öffnende Klammer.");
    }
...
function removeCustomPropertyDeclarations(source, properties) {
  let output = source;
...
designSystem = removeMediaBlocksByPattern(
  designSystem,
  /@media\s*\(\s*prefers-color-scheme\s*:\s*dark\s*\)\s*\{/g,
  (block) =>
    legacyThemeProperties.some((property) =>
      block.includes(`${property}:`)
    )
);

/*
 * Defensive cleanup: the old file contains more than one historical token
 * declaration block. All legacy theme declarations are removed globally,
 * while usages such as var(--pt-theme-text) remain intact and resolve through
 * foundation/tokens.css.
 */
designSystem = removeCustomPropertyDeclarations(
  designSystem,
  legacyThemeProperties
);
...
if (
  ...
  throw new Error(
    "Mindestens eine konkurrierende --pt-theme-Definition blieb im Design-System zurück."
  );
}

if (beforeDesignSystem === designSystem) {
  throw new Error(
    "Keine konkurrierende Theme-Definition wurde entfernt. " +
