...
const COMPARISON_ASSET_TARGET =
  "apps/pfotentechnik/src/assets/images/project/pfotentechnik/comparison";

const COMPARISON_ASSET_CANDIDATES = [
  "src/assets/images/project/pfotentechnik/comparison",
  "files/apps/pfotentechnik/src/assets/images/project/pfotentechnik/comparison",
  "files/src/assets/images/project/pfotentechnik/comparison"
];

const REQUIRED_HERO_SLUGS = [
...
const listEditorialHeroes = async (relativeDirectory) => {
  try {
    const entries = await readdir(join(root, relativeDirectory), { withFileTypes: true });
    return entries
      .filter((entry) => entry.isFile() && entry.name.endsWith("-editorial-hero.webp"))
...
const recoverComparisonHeroAssets = async () => {
  const targetAbsolute = join(root, COMPARISON_ASSET_TARGET);
  await mkdir(targetAbsolute, { recursive: true });

  const copied = [];

  for (const candidate of COMPARISON_ASSET_CANDIDATES) {
    const files = await listEditorialHeroes(candidate);

    for (const filename of files) {
      const source = join(root, candidate, filename);
      const destination = join(targetAbsolute, filename);

      try {
        await access(destination, constants.F_OK);
      } catch {
        await copyFile(source, destination);
        copied.push({ filename, source: candidate });
      }
    }
  }

  const available = await listEditorialHeroes(COMPARISON_ASSET_TARGET);
  const missing = REQUIRED_HERO_SLUGS.filter(
    (slug) => !available.includes(`${slug}-editorial-hero.webp`)
  );
...
<header class="site-header-v2" data-site-header data-header-version="15.4.4">
...
.comparison-cover__image {
  display: block;
  width: 100%;
  height: 100%;
  border-radius: inherit;
  object-fit: cover;
  clip-path: inset(0 round var(--pt-radius-xl));
}
...
const heroAssetRecovery = await recoverComparisonHeroAssets();

await write(
  "apps/pfotentechnik/reports/comparison-platform/hero-assets-15.4.4.json",
  JSON.stringify(
    {
      generatedAt: new Date().toISOString(),
      targetDirectory: COMPARISON_ASSET_TARGET,
      copied: heroAssetRecovery.copied,
      available: heroAssetRecovery.available,
      missing: heroAssetRecovery.missing
    },
    null,
    2
  ) + "\n"
...
for (const item of heroAssetRecovery.copied) {
  console.log(`Hero-Asset übernommen: ${item.filename} ← ${item.source}`);
}

if (heroAssetRecovery.missing.length > 0) {
  console.warn(
    `[${LABEL}] Fehlende individuelle Hero-Assets: ${heroAssetRecovery.missing.join(", ")}`
  );
  console.warn("Für diese Slugs bleibt der Default-Hero aktiv.");
}
...
shell = shell.replace(
  ...
  'data-comparison-cover-version="15.4.4"'
);
...
if (!writtenCss.includes("overflow: clip") || !writtenCss.includes("clip-path: inset(0 round")) {
  throw new Error("Vollständige Hero-Rundung fehlt.");
}
if (!writtenCss.includes("min-height: 2.8rem")) {
  throw new Error("Mobile Filter wurden nicht verdichtet.");
}

const gpsHeroPath = join(
  root,
  COMPARISON_ASSET_TARGET,
  "beste-gps-tracker-fuer-hunde-editorial-hero.webp"
);

try {
  const gpsHeroStat = await stat(gpsHeroPath);
  if (gpsHeroStat.isFile() && gpsHeroStat.size > 0) {
    console.log("GPS-Hero gefunden:", gpsHeroPath.replace(`${root}/`, ""));
  }
} catch {
  console.warn(
    `[${LABEL}] beste-gps-tracker-fuer-hunde-editorial-hero.webp ist weiterhin nicht im App-Asset-Ordner vorhanden.`
  );
}
...
console.log("- falsch abgelegte Comparison-Hero-Assets werden übernommen");
console.log("- Hero-Bild vollständig rund inklusive Unterkante");
console.log("- mobile Filter deutlich kompakter");
