...
const PATCH = "pfotentechnik-comparison-desktop-polish-32.4.0";
...
const testFile = path.join(root, "apps", "pfotentechnik", "test", "pfotentechnik-comparison-desktop-polish-32.4.0.test.mjs");
...
run(process.execPath, ["--check", heroFile]);
run(process.execPath, ["--check", priceFile]);
run(process.execPath, ["--check", shellFile]);
run(process.execPath, ["--test", testFile]);

console.log(`[${PATCH}] Vergleichs-Desktop-Polish erfolgreich angewendet.`);
