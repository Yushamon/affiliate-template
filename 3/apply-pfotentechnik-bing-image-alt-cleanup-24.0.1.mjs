...
const NAME = "pfotentechnik-bing-image-alt-cleanup-24.0.1";
...
function parsePackageJson(source) {
  ...
  try {
    return { pkg: JSON.parse(normalized), repaired: false };
  } catch (originalError) {
    const withoutTrailingCommas = normalized.replace(/,\s*([}\]])/g, "$1");
    try {
      return { pkg: JSON.parse(withoutTrailingCommas), repaired: true };
    } catch {
      const error = new Error(
        "package.json ist ungültig und konnte nicht sicher automatisch repariert werden: " +
...
function updatePackage(changedFiles, operations) {
  ...
  const { pkg, repaired } = parsePackageJson(source);
  pkg.scripts ??= {};
  const expected = {
  ...
  operations.push({
    file: relative(PACKAGE_JSON),
    status: CHECK ? "required" : "changed",
    description: repaired
      ? "Ungültiges JSON reparieren und Audit-/Test-Skripte ergänzen"
      : "Audit- und Test-Skripte ergänzen"
  });
  if (CHECK) return;
  backup(PACKAGE_JSON);
