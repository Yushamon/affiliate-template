#!/usr/bin/env node
/**
 * pfotentechnik-katzenklappen-product-md-normalization-30.2.1
 *
 * Hotfix:
 * - robuster Repository-Root (package.json wird nach oben gesucht)
 * - robuster Kategorienvergleich
 * - Test verlangt >=1 Katzenklappe statt >=3
 * - zusätzliche Assertions für affiliate/price/manufacturer
 */
...
function findRepoRoot(start) {
  let dir = start;
  while (true) {
    const pkg = path.join(dir, "apps", "pfotentechnik", "package.json");
    if (fs.existsSync(pkg)) return dir;
    const parent = path.dirname(dir);
    if (parent === dir) throw new Error("Repository-Root konnte nicht gefunden werden.");
    dir = parent;
  }
}
...
const TEST = path.join(APP, "test", "katzenklappen-product-md-normalization-30.2.1.test.mjs");
...
pkg.scripts??={};
pkg.scripts["test:katzenklappen-md-normalization"]="node --test test/katzenklappen-product-md-normalization-30.2.1.test.mjs";
fs.writeFileSync(pkgPath,JSON.stringify(pkg,null,2)+"\n");
console.log("30.2.1 Hotfix installiert.");
