...
const NAME = "pfotentechnik-css-base-layer-cleanup-22.2.1";
...
const REPORT_JSON = path.join(REPORT_DIR, "css-base-layer-22.2.1.json");
const REPORT_MD = path.join(REPORT_DIR, "css-base-layer-22.2.1.md");
...
function cleanupBlock(block) {
  ...
  return {
    text:
      normalizeSelector(block.header) +
      " {\n" +
...
const indexAfter = updateIndex(fs.readFileSync(INDEX, "utf8"));
...
const report = {
  ...
  note:
    "Nur der direkt nach dem Foundation-Import liegende globale Präfix wurde migriert. " +
    "Spätere Theme-, Dark-Mode- und Override-Regeln bleiben absichtlich an ihrer Kaskadenposition.",
  generatedAt: new Date().toISOString()
};

const reportMarkdown = `# CSS Base Layer 22.2.1
