#!/usr/bin/env node
/**
 * PfotenTechnik Vergleichsplattform Refactor 10.0.2
 *
 ...
 *
 * Nutzung:
 *   node pfotentechnik-comparison-platform-refactor-10.0.2.mjs
 *   node pfotentechnik-comparison-platform-refactor-10.0.2.mjs --check
 *   node pfotentechnik-comparison-platform-refactor-10.0.2.mjs --push
 *   node pfotentechnik-comparison-platform-refactor-10.0.2.mjs --no-commit
 *   node pfotentechnik-comparison-platform-refactor-10.0.2.mjs --allow-dirty
 *   node pfotentechnik-comparison-platform-refactor-10.0.2.mjs --branch=<name>
 */
...
const restoreGeneratedAndHistoricalSnapshots = async () => {
  const commit = String(
    git(
      [
        "log",
        "--format=%H",
        "--grep=^fix\\(seo\\): canonicalize comparison routes and internal links$",
        "-n",
        "1"
      ],
  ...
  const paths = [
    ...
    "apps/pfotentechnik/src/data/seo/gsc-dashboard-ranges.json",
    "apps/pfotentechnik/src/data/seo/gsc-dashboard.json",
    "apps/pfotentechnik/src/data/seo/search-dashboard-ranges.json",
    "apps/pfotentechnik/src/data/seo/search-dashboard.json"
  ];

  for (const relative of paths) {
    const result = git(
      ["show", `${parent}:${relative}`],
      { capture: true, allowFailure: true }
    );
    if (result.status !== 0) continue;
    await writeText(path.join(ROOT, relative), result.stdout);
  }
};
...
const rewriteInternalLinks = async (mapping) => {
  ...
  const files = (
    ...
    .filter((file) => {
      const relative = relativeToRoot(file).replaceAll("\\", "/");
      return !relative.startsWith("apps/pfotentechnik/src/generated/") &&
        !relative.startsWith("apps/pfotentechnik/src/data/seo/");
    });
  ...
  for (const file of files) {
    ...
    for (const [from, to] of replacements) {
      ...
      for (const variant of variants) {
        ...
        const routeBoundary = `(?=$|[?#"'\\\\s)\\\\]\\\\}>,])`;
        const pattern = isRootAlias
          ? new RegExp(
              `(?<!/vergleiche)${escapedVariant}${routeBoundary}`,
              "g"
            )
          : new RegExp(`${escapedVariant}${routeBoundary}`, "g");
...
const main = async () => {
  ...
  await restoreGeneratedAndHistoricalSnapshots();
