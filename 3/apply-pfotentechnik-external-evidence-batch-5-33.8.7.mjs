...
const PATCH = "pfotentechnik-external-evidence-batch-5-33.8.7";
...
const testFile = path.join(app, "test", "external-evidence-batch-5-33.8.7.test.mjs");
...
const replaceExternal = (raw, replacement, slug) => {
  ...
  return [...lines.slice(0, start), ...replacement.trimEnd().split("\n"), ...lines.slice(end)].join("\n") + (raw.endsWith("\n") ? "\n" : "");
};
...
try {
  for (const [slug, block] of Object.entries(blocks)) {
    ...
    if (!raw.includes(expectedRatings[slug])) throw new Error(`Erwartetes PfotenTechnik-Rating fehlt vor Patch: ${slug}`);

    const backupFile = path.join(backup, path.relative(root, file));
    fs.mkdirSync(path.dirname(backupFile), { recursive: true });
    fs.copyFileSync(file, backupFile);

    const next = replaceExternal(raw, block, slug);
    if (!next.includes(expectedRatings[slug])) throw new Error(`PfotenTechnik-Rating nach Patch verändert: ${slug}`);
...
`;
  ...
  console.log(`[${PATCH}] Oneisall bleibt bewusst partial.`);
