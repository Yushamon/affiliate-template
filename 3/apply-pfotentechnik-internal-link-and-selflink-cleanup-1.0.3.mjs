#!/usr/bin/env node
/**
 * PfotenTechnik Internal Link + Selflink Cleanup 1.0.3
 *
 * Ausführung im Root von Yushamon/affiliate-template:
 *   node apply-pfotentechnik-internal-link-and-selflink-cleanup-1.0.3.mjs
 *
 * Optionen:
...
for (const file of htmlFiles) {
  ...
  const routeFromFile = path.relative(dist, file).replace(/\\/g, "/").replace(/(?:^|\/)index\.html$/, "").replace(/^/, "/").replace(/\/+$/, "/") || "/";
  const isAdminRoute = routeFromFile === "/admin/" || routeFromFile.startsWith("/admin/");
  if (!canonical && !isAdminRoute) add("error","CANONICAL_MISSING",{file:path.relative(root,file)});
