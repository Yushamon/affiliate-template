#!/usr/bin/env node
/**
 * PfotenTechnik Product Light Mode + Category Link 4.1.1
 *
 * Usage:
 *   node apply-pfotentechnik-product-lightmode-category-4.1.1.mjs --check
 *   node apply-pfotentechnik-product-lightmode-category-4.1.1.mjs
 */
...
const RENDERER_START = "/* product-lightmode-category-4.1.1 */";
const RENDERER_END = "/* end product-lightmode-category-4.1.1 */";
const NEXT_START = "/* decision-next-steps-theme-4.1.1 */";
const NEXT_END = "/* end decision-next-steps-theme-4.1.1 */";
...
/*
 * Kategoriepfad direkt aus dem gepflegten Frontmatter verwenden.
 * Kein pauschaler Futterautomaten-Fallback.
 */
...
if (rendererNext.includes(oldBadge)) {
  rendererNext = rendererNext.replace(oldBadge, newBadge);
...
${RENDERER_START}
  ...
  /*
   * Heller Score-Chip statt dunkler Typografie auf gesättigtem Grün.
   * Die vollständige Bewertung bleibt als zusammenhängende Einheit lesbar.
   */
  .native-product__score {
...
let nextStepsNext = removeBlock(nextOriginal, NEXT_START, NEXT_END);

const nextCss = `
${NEXT_START}
  /*
   * recommendation-cards-2.1 setzte bisher immer ein dunkles Farbschema.
   * Diese Schicht definiert Light Mode als Standard und Dark Mode explizit.
   */
