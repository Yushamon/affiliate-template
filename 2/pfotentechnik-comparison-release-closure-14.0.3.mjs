...
const NAME = "pfotentechnik-comparison-release-closure-14.0.3";
...
const diagnosticFile = path.join(
  process.cwd(),
  "pfotentechnik-comparison-release-closure-14.0.3-error.log"
);
...
const files = {
  ...
  test: path.join(appRoot, "test", "comparison-release-closure-14.0.3.test.mjs"),
  report: path.join(appRoot, "reports", "comparison-platform", "comparison-release-closure-installer-14.0.3.md")
};
...
function statementEndAt(source, startIndex, label) {
  let paren = 0;
  let bracket = 0;
  let brace = 0;
  let quote = null;
  ...
  fail(new Error(`Statement-Ende nicht gefunden: ${label}`));
}

function patchRecommendationFallback(source) {
  if (
    source.includes("const eligibleItemSlugs = items") &&
    source.includes("eligibleItemSlugs.find((slug) => slug !== resolvedWinnerSlug)")
  ) {
    return source;
  }

  const startMatch = /\bconst\s+winnerCandidate\s*=/m.exec(source);
  if (!startMatch) {
    fail(new Error("winnerCandidate-Deklaration im ViewModel nicht gefunden."));
  }

  const alternativeMatch = /\bconst\s+resolvedAlternativeSlug\s*=/m.exec(
    source.slice(startMatch.index)
  );
  if (!alternativeMatch) {
    fail(new Error("resolvedAlternativeSlug-Deklaration im ViewModel nicht gefunden."));
  }

  const alternativeStart = startMatch.index + alternativeMatch.index;
  const end = statementEndAt(
    source,
    alternativeStart,
    "resolvedAlternativeSlug"
  );
  const start = lineStartAt(source, startMatch.index);

  const replacement = `  const eligibleItemSlugs = items
    .filter((item) =>
      item.type === "product" &&
...
function patchVisibleRows(source) {
  if (
    source.includes("const rowCandidates = rawRows.map") &&
    source.includes("row.resolvedCount === row.cells.length")
  ) {
    return source;
  }

  const startMatch =
    /\bconst\s+rows\s*:\s*ComparisonRow\[\]\s*=\s*rawRows\.map\b/m.exec(source);

  if (!startMatch) {
    fail(new Error(
      "rows-Deklaration im ViewModel nicht gefunden. " +
      "Erwartet wird eine aus rawRows erzeugte ComparisonRow-Liste."
    ));
  }

  const boundaryMatch =
    /\n\s*const\s+filterValuesBySlug\b/m.exec(source.slice(startMatch.index));

  if (!boundaryMatch) {
    fail(new Error("filterValuesBySlug-Grenze im ViewModel nicht gefunden."));
  }

  const start = lineStartAt(source, startMatch.index);
  const end = startMatch.index + boundaryMatch.index + 1;

  const replacement = `  const rowCandidates = rawRows.map((row) => {
...
function addAuditErrorCodes(source) {
  const requiredCodes = [
  ...
  if (requiredCodes.every((code) => source.includes(`"${code}"`))) {
    return source;
  }

  const markerIndex = source.indexOf('"RECOMMENDATION_DUPLICATE"');
  if (markerIndex === -1) {
    fail(new Error("Audit-Fehlercodeliste konnte nicht lokalisiert werden."));
  }

  let closeIndex = source.indexOf("]", markerIndex);
  if (closeIndex === -1) {
    fail(new Error("Ende der Audit-Fehlercodeliste wurde nicht gefunden."));
  }

  const missing = requiredCodes.filter(
    (code) => !source.includes(`"${code}"`)
  );
  const insertion = missing.map((code) => `,\n  "${code}"`).join("");

  return source.slice(0, closeIndex) + insertion + source.slice(closeIndex);
}

function insertAuditRowCoverage(source, block) {
  if (source.includes("const fullyResolvedRows")) return source;

  const winnerMatch =
    /\bconst\s+winner\s*=\s*d\.recommendation\?\.winnerSlug\s*;/m.exec(source);

  if (!winnerMatch) {
    fail(new Error("winner-Deklaration im Comparison-Audit nicht gefunden."));
  }

  const start = lineStartAt(source, winnerMatch.index);
  return source.slice(0, start) + block + "\n" + source.slice(start);
}

function addAuditEligibilityChecks(source) {
  if (
    source.includes('addIssue(issues, "WINNER_INELIGIBLE"') &&
    source.includes('addIssue(issues, "ALTERNATIVE_INELIGIBLE"')
  ) {
    return source;
  }

  const duplicateMatch =
    /if\s*\(\s*winner\s*&&\s*alternative\s*&&\s*winner\s*===\s*alternative\s*\)\s*addIssue\([\s\S]{0,400}?RECOMMENDATION_DUPLICATE[\s\S]{0,400}?\);/m.exec(source);

  if (!duplicateMatch) {
    fail(new Error(
      "RECOMMENDATION_DUPLICATE-Prüfung im Comparison-Audit nicht gefunden."
    ));
  }
  ...
  return (
    source.slice(0, duplicateMatch.index + duplicateMatch[0].length) +
    insertion +
    source.slice(duplicateMatch.index + duplicateMatch[0].length)
  );
}

function addMalformedComparisonAudit(source) {
  if (source.includes("MALFORMED_COMPARISON_PATH")) return source;

  const scanStart = source.indexOf("for (const rootDir of scanRoots)");
  if (scanStart === -1) {
    fail(new Error("scanRoots-Schleife im Refactor-Audit nicht gefunden."));
  }

  const readMatch =
    /const\s+source\s*=\s*await\s+readFile\(\s*file\s*,\s*["']utf8["']\s*\)\s*;/m.exec(
      source.slice(scanStart)
    );

  if (!readMatch) {
    fail(new Error("Datei-Leseanker im Refactor-Audit nicht gefunden."));
  }

  const end = scanStart + readMatch.index + readMatch[0].length;
  const insertion = `
...
viewModel = patchRecommendationFallback(viewModel);
viewModel = patchVisibleRows(viewModel);
...
audit = addAuditErrorCodes(audit);
...
audit = insertAuditRowCoverage(audit, auditRowBlock);

audit = addAuditEligibilityChecks(audit);
...
refactorAudit = addMalformedComparisonAudit(refactorAudit);
...
log("Comparison Release Closure 14.0.3 technisch abgeschlossen.");
