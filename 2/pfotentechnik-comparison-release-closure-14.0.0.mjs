...
const NAME = "pfotentechnik-comparison-release-closure-14.0.0";
...
const files = {
  ...
  test: path.join(appRoot, "test", "comparison-release-closure-14.0.0.test.mjs"),
  report: path.join(appRoot, "reports", "comparison-platform", "comparison-release-closure-installer-14.0.0.md")
};
...
function patchResolverTs(source) {
  ...
  if (!output.includes("comparisonAliasCandidates")) {
  ...
  output = output.replace(
    /const candidates = new Set\(\[\s*normalized,\s*normalizeKey\(criterion\.label\),\s*\.\.\.\(aliases\[normalized\] \?\? \[\]\)\s*\]\);/m,
    "const candidates = comparisonAliasCandidates(normalized, criterion.label);"
  );

  if (!output.includes('case "mindestportion":')) {
    output = replaceRequired(
      output,
...
const markdown = [
  "# Comparison Release Closure 14.0.0",
  "",
  \`Erstellt: \${report.generatedAt}\`,
  "",
...
console.log("Comparison Release Closure 14.0.0");
...
log("Comparison Release Closure 14.0.0 technisch abgeschlossen.");
