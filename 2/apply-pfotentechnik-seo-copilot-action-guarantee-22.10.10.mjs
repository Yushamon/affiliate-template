...
const NAME = "pfotentechnik-seo-copilot-action-guarantee-22.10.10";
...
const REPORT = path.join(APP, "reports", "seo-copilot", "action-guarantee-22.10.10.md");
...
const registryImportPattern =
  /import\s*\{([\s\S]*?)\}\s*from\s*["']\.\/ai-action-registry\.mjs["'];?/m;

const registryImportMatch = findingAi.match(registryImportPattern);

if (!registryImportMatch) {
  throw new Error("Import aus ai-action-registry.mjs wurde nicht gefunden.");
}

const registryNames = registryImportMatch[1]
  .split(",")
  .map((name) => name.trim())
  .filter(Boolean);

for (const requiredName of ["getAiActionDefinition", "resolveFindingAiActionIds"]) {
  if (!registryNames.includes(requiredName)) registryNames.push(requiredName);
}

findingAi = findingAi.replace(
  registryImportPattern,
  `import { ${registryNames.join(", ")} } from "./ai-action-registry.mjs";`,
);

if (
  !findingAi.includes("getAiActionDefinition") ||
  !findingAi.includes("resolveFindingAiActionIds")
) {
  throw new Error("Import aus ai-action-registry.mjs konnte nicht verifiziert werden.");
}

const slugFunctionPattern =
  /const\s+slugFromRoute\s*=\s*\(route:\s*string\)\s*=>\s*\{[\s\S]*?\n\};/m;

const safeSlugFunction = `const slugFromRoute = (route?: string) => {
  if (typeof route !== "string" || !route.trim()) return undefined;
  const match = route.match(/^\\/(?:produkt|vergleiche)\\/([^/]+)\\/?$/);
  return match?.[1];
};`;

if (slugFunctionPattern.test(findingAi)) {
  findingAi = findingAi.replace(slugFunctionPattern, safeSlugFunction);
} else if (!findingAi.includes('typeof route !== "string" || !route.trim()')) {
  throw new Error("slugFromRoute konnte nicht sicher aktualisiert werden.");
}
...
const reportContent = `# SEO Copilot Action Guarantee 22.10.10
