...
const NAME = "pfotentechnik-visual-generation-engine-2.3.0";
...
const TEST = path.join(APP, "test", "visual-generation-engine-2.3.0.test.mjs");
...
let growth = fs.readFileSync(GROWTH, "utf8");
growth = insertOnce(growth, 'import path from "node:path";\n', 'import { buildVisualGenerationPlan, type VisualGenerationPlan } from "./visual-generation";\n', "growth import", "after");
growth = insertOnce(growth, '  implementationPrompt: string;\n', '  visualPlan: VisualGenerationPlan;\n  visualPrompt: string;\n', "growth type", "after");
growth = replaceOnce(growth, '            const implementationBrief = buildImplementationBrief(item);\nreturn {', '      const implementationBrief = buildImplementationBrief(item);\n      const visualPlan = buildVisualGenerationPlan(item);\n      return {', "growth map");
growth = insertOnce(growth, '        implementationPrompt: buildResearchImplementationPrompt(item, implementationBrief),\n', '        visualPlan,\n        visualPrompt: visualPlan.masterPrompt,\n', "growth values", "after");
...
let workbench = fs.readFileSync(WORKBENCH, "utf8");
...
pkg.scripts ??= {};
pkg.scripts["test:visual-generation"] = "node --experimental-strip-types --test test/visual-generation-engine-2.3.0.test.mjs";
