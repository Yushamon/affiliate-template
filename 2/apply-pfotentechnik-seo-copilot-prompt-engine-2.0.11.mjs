...
const NAME = "pfotentechnik-seo-copilot-prompt-engine-2.0.11";
...
try {
  ...
  const semanticMarketSignalLines = [
  ...
  const corruptedSemanticAssertion =
    semanticMarketSignalLines.join("\\n");

  let updatedSeoCopilotTest = seoCopilotTestSource;
  let testRepairMode = "unchanged";

  if (legacyMarketSignalAssertion.test(updatedSeoCopilotTest)) {
    updatedSeoCopilotTest = updatedSeoCopilotTest.replace(
      legacyMarketSignalAssertion,
      semanticMarketSignalAssertion,
    );
    testRepairMode = "legacy-assertion-migrated";
  } else if (updatedSeoCopilotTest.includes(corruptedSemanticAssertion)) {
    updatedSeoCopilotTest = updatedSeoCopilotTest.replace(
      corruptedSemanticAssertion,
      semanticMarketSignalAssertion,
    );
    testRepairMode = "literal-newlines-repaired";
  } else if (updatedSeoCopilotTest.includes(semanticMarketSignalAssertion)) {
    testRepairMode = "already-correct";
  } else {
    throw new Error(
      "Marktsignal-Test entspricht weder Alt-, Defekt- noch Zielzustand. Repository-Stand manuell prüfen.",
    );
  }

  if (updatedSeoCopilotTest !== seoCopilotTestSource) {
    fs.writeFileSync(seoCopilotTestPath, updatedSeoCopilotTest, "utf8");
    log(
      `Gepatcht: apps/pfotentechnik/test/seo-copilot.test.mjs (${testRepairMode})`,
    );
  } else {
    log(
      `Unverändert: apps/pfotentechnik/test/seo-copilot.test.mjs (${testRepairMode})`,
    );
  }
