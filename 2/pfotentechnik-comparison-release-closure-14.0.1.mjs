...
const NAME = "pfotentechnik-comparison-release-closure-14.0.1";
...
const files = {
  ...
  test: path.join(appRoot, "test", "comparison-release-closure-14.0.1.test.mjs"),
  report: path.join(appRoot, "reports", "comparison-platform", "comparison-release-closure-installer-14.0.1.md")
};
...
function findAliasesObjectRange(source, label) {
  const declaration = /const\s+aliases(?:\s*:\s*[^=\n]+)?\s*=\s*\{/m.exec(source);
  if (!declaration) fail(`Aliasobjekt nicht gefunden: ${label}`);
  ...
  fail(`Aliasobjekt ist nicht abgeschlossen: ${label}`);
}
...
function insertBeforeAsRecord(source, helper, label) {
  ...
  const match = /\nconst\s+asRecord\b/m.exec(source);
  if (!match) fail(`asRecord-Anker nicht gefunden: ${label}`);
...
function replaceCandidateBuilder(source, label) {
  ...
  const pattern = /const\s+candidates\s*=\s*new\s+Set\(\s*\[\s*normalized\s*,[\s\S]{0,500}?\]\s*\);/m;
  if (!pattern.test(source)) fail(`Kandidatenaufbau nicht gefunden: ${label}`);
  return source.replace(
    pattern,
...
function insertSwitchCases(source, snippet, label) {
  ...
  if (!anchor) fail(`Switch-Anker nicht gefunden: ${label}`);
...
write(files.resolverTs, patchResolverTs(read(files.resolverTs)));
write(files.resolverMjs, patchResolverMjs(read(files.resolverMjs)));
...
log("Comparison Release Closure 14.0.1 technisch abgeschlossen.");
