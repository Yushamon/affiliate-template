...
const PATCH_NAME = "pfotentechnik-editorial-transparency-20.1.1";
...
const installedPagePattern =
  /<DecisionJourney[\s\S]*?<EditorialTransparency kind="ratgeber" \/>\s*<FAQ items=\{assembledPage\.faq\} \/>/;

const knownPartialPagePattern =
  /<DecisionJourney[\s\S]*?<EditorialTransparency(?:(?!<EditorialTransparency)[\s\S])*?kind="ratgeber"(?:(?!<EditorialTransparency)[\s\S])*?\/>\s*<FAQ items=\{assembledPage\.faq\} \/>/;
...
const auditIsInstalled =
  original.audit.includes("Ratgeber behalten die Decision Journey vor der kompakten Transparenz");
...
const baselineState = {
  ...
  audit:
    ...
    auditIsInstalled ||
    original.audit.includes("Ratgeber nutzen die kompakte, einklappbare Transparenzdarstellung")
};
...
const moveGuideTransparency = (source) => {
  ...
  if (installedPagePattern.test(source)) return source;
...
const currentAuditBlock = `requirePattern(
  "src/pages/[slug].astro",
  /<Content \\/>[\\s\\S]*?<DecisionJourney[\\s\\S]*?<EditorialTransparency kind="ratgeber" \\/>[\\s\\S]*?<FAQ items=\\{assembledPage\\.faq\\} \\/>/,
...
const updateAudit = (source) => {
  if (source.includes("Ratgeber behalten die Decision Journey vor der kompakten Transparenz")) {
    return source;
  }

  if (source.includes(previousPatchedAuditBlock)) {
    return source.replace(previousPatchedAuditBlock, currentAuditBlock);
  }

  if (source.includes(originalAuditBlock)) {
    return source.replace(originalAuditBlock, currentAuditBlock);
  }
  ...
  throw new Error(
    "Das Editorial-Transparency-Audit entspricht keinem geprüften Stand."
  );
};
...
const auditIsValid =
  next.audit.includes("Ratgeber behalten die Decision Journey vor der kompakten Transparenz") &&
  next.audit.includes("Ratgeber nutzen die kompakte, einklappbare Transparenzdarstellung");
