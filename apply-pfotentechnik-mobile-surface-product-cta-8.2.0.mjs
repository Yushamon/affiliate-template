...
const PATCH_ID = "pfotentechnik-mobile-surface-product-cta-8.2.0";
...
const PREMIUM_MARKER =
  "/* === PfotenTechnik Mobile Surface Product CTA 8.2.0 === */";

const PREMIUM_MARKER_PATTERN =
  /\/\* === PfotenTechnik Mobile Surface Product CTA 8\.(?:1\.[34]|2\.0) === \*\//;
...
function patchPremiumRenderer(source) {
  if (PREMIUM_MARKER_PATTERN.test(source)) {
    return {
      content: source,
...
/* === End PfotenTechnik Mobile Surface Product CTA 8.2.0 === */
`,
    eol
  );
...
function patchProductRenderer(source) {
  ...
  if (!next.includes('class="native-buybox__secondary"')) {
    fail("ProductRenderer: sekundärer Buybox-CTA nicht gefunden.");
  }

  if (!next.includes("const secondaryCtaHref =")) {
    const anchorPattern = /(\r?\n)(const scoreLabel\s*=)/;

    if (!anchorPattern.test(next)) {
      fail("ProductRenderer: Einfügepunkt vor scoreLabel nicht gefunden.");
    }
    ...
    next = next.replace(
      anchorPattern,
      `${eol}${insertion}${eol}${eol}$2`
    );

    changes.push("Sekundärer CTA erhält immer ein echtes Ziel");
  }

  const secondaryCtaPattern =
    /<a\b(?=[^>]*class="native-buybox__secondary")[^>]*>[\s\S]*?<\/a>/;

  const secondaryCtaMatch = next.match(secondaryCtaPattern);

  if (!secondaryCtaMatch) {
    fail("ProductRenderer: sekundärer Buybox-CTA konnte nicht gelesen werden.");
  }

  if (!secondaryCtaMatch[0].includes("href={secondaryCtaHref}")) {
    next = next.replace(
      secondaryCtaPattern,
    ...
    changes.push("Toter oder statischer Alternativen-Link ersetzt");
  }

  if (/\bconst testStatus\s*=/.test(next)) {
    const statusVariablePattern =
      /(?:\r?\n)const testStatus\s*=\s*asText\([\s\S]*?\r?\n\);\r?\n/;

    if (!statusVariablePattern.test(next)) {
      fail(
        "ProductRenderer: Statusvariable erkannt, aber ihre Struktur ist unbekannt."
      );
    }

    next = next.replace(statusVariablePattern, eol);
    changes.push("Nicht mehr benötigte Statusvariable entfernt");
  }

  const statusRowPattern =
    /\s*\{testStatus\s*&&\s*<div>\s*<dt>Status<\/dt>\s*<dd>\{testStatus\}<\/dd>\s*<\/div>\}/;

  if (statusRowPattern.test(next)) {
    next = next.replace(statusRowPattern, "");
  ...
  if (!next.includes("href={secondaryCtaHref}")) {
    fail("ProductRenderer: dynamisches CTA-Ziel wurde nicht eingesetzt.");
  }

  if (/\btestStatus\b/.test(next)) {
    fail("ProductRenderer: Statusreferenz ist noch vorhanden.");
  }
...
const ALTERNATIVES_CONTENT = `/* PfotenTechnik Product Alternatives 8.2.0 */
...
function main() {
  ...
  log(`Repository: ${root}`);
  log("Abgleich: Yushamon/affiliate-template, Branch main, Stand 2026-07-24");
