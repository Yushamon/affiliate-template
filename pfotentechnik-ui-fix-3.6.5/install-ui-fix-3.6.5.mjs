...
const source = path.join(packageRoot, "ui-fix-3.6.5.css");
const backup = `${target}.before-ui-fix-3.6.5`;

const startMarker = "/* === PfotenTechnik UI Fix 3.6.5 === */";
const endMarker = "/* === End PfotenTechnik UI Fix 3.6.5 === */";
...
if (!fs.existsSync(source)) {
  console.error("ui-fix-3.6.5.css fehlt im Paket.");
  process.exit(1);
}
...
for (const entry of required) {
  if (!verification.includes(entry)) {
    ...
    console.error("Fehlend:", entry);
    process.exit(1);
  }
}

console.log("PfotenTechnik UI Fix 3.6.5 installiert.");
