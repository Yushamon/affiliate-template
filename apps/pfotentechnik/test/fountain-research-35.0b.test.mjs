...
test('conflicting interval and zero JS placeholder do not produce annual costs',()=>{const baseline=JSON.parse(fs.readFileSync(new URL('../../../reports/seo-cockpit/fountain-cost-35.0b.1-evidence/baseline-dataset.json',import.meta.url),'utf8'));const rows=inspectDataset(baseline).rows;for(const slug of ['oneisall-7l-dog-water-fountain','xiaomi-smart-pet-fountain-2']){const c=rows.find(r=>r.slug===slug).annualFilterCost;assert.equal(c.status,'insufficientData');assert.equal(c.annualCostLow,null);}});
