...
test("alle elf automatischen Katzentoiletten besitzen normalisierte Streudaten", () => {
  ...
  for (const product of litter) {
    assert.ok(["complete", "partial", "unknown"].includes(product.litterCompatibility.status));
    assert.ok(Array.isArray(product.litterCompatibility.compatibleTypes));
    assert.ok(Array.isArray(product.litterCompatibility.incompatibleTypes));
    assert.ok(Array.isArray(product.litterCompatibility.evidence));
...
test("unknown bleibt ein eigener Status und ungestuetzte Individualclaims werden abgelehnt", () => {
  assert.deepEqual(validateDecisionData({ slug: "unknown-ok", litterCompatibility: {
    status: "unknown", compatibleTypes: [], conditionalTypes: [], incompatibleTypes: [], clumpingRequirement: "unknown", evidence: []
  } }), []);
