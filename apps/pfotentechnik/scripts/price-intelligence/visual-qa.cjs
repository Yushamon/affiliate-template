...
const server=http.createServer(async(req,res)=>{
  ...
  if(url.pathname.startsWith('/api/')){
    ...
    else if(req.method==='POST')result={ok:true,record,providerResults:[{id:'petlibro-de',ok:true}]};
...
app.whenReady().then(async()=>{
  ...
  win.webContents.on('console-message',(_e,level,message)=>{if(level===3&&!message.includes('ERR_CONNECTION_REFUSED'))report.errors.push(message);});
  const js=source=>win.webContents.executeJavaScript(source);
  const settle=()=>js('Promise.all([document.fonts.ready, new Promise(r=>setTimeout(r,250))])');
  for(const theme of ['light','dark']){
    nativeTheme.themeSource=theme;
    for(const width of [375,1600,320,390,768,1024,1280]){
      win.setContentSize(width,900);await win.loadURL(base+route);await settle();
      // Load the entire page's original lazy images before the one full-page capture.
      await js(`Promise.all([...document.images].map(i=>{i.loading='eager';return i.decode().catch(()=>{});}));`);
      const result=await js('('+inspect.toString()+')()');report.checks.push({kind:'public',theme,...result});
  ...
  for(const width of [375,768,1600]){
    ...
    const geometry=await js(`({overflow:Math.max(0,document.documentElement.scrollWidth-innerWidth),visible:[...document.querySelectorAll('[data-product-row]')].filter(e=>!e.hidden).length,fields:[...document.querySelectorAll('[data-product-row]:not([hidden]) [data-offer-editor] input')].every(e=>e.getBoundingClientRect().right<=innerWidth+1)})`);
  ...
  report.passed=report.checks.every(c=>!c.overflow&&!c.badCtas&&!c.badPrices&&!c.overlaps&&!c.brokenImages&&c.fields!==false)&&report.draftPreserved&&['/check','/check-all','/offer'].every(suffix=>report.apiRequests.some(r=>r.path.endsWith(suffix)));
