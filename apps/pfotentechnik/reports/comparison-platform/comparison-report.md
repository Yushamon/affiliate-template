# Comparison Platform Report

Erstellt: 2026-08-15T20:52:44.160Z

## Übersicht

- Vergleiche: 28
- Produkte: 85
- Hersteller: 26
- verwendete Produkte: 56
- Produktabdeckung: 65.9 %
- Qualitätsscore: 98/100
- Fehler: 0
- Warnungen: 4

## Häufigste Befunde

- **PRODUCT_NOT_COVERED**: 4 (warning, coverage)

## Dateien mit den meisten Problemen

- `src/content/products/enabot-ebo-air-2.md`: 0 Fehler, 1 Warnungen
- `src/content/products/furbo-360-hundekamera.md`: 0 Fehler, 1 Warnungen
- `src/content/products/litter-robot-5-pro.md`: 0 Fehler, 1 Warnungen
- `src/content/products/neakasa-m1-plus.md`: 0 Fehler, 1 Warnungen

## Fehler

Keine Fehler.

## Warnungen

- **PRODUCT_NOT_COVERED** – `src/content/products/enabot-ebo-air-2.md`: Aktives Produkt kommt in keiner Vergleichsseite vor.
- **PRODUCT_NOT_COVERED** – `src/content/products/furbo-360-hundekamera.md`: Aktives Produkt kommt in keiner Vergleichsseite vor.
- **PRODUCT_NOT_COVERED** – `src/content/products/litter-robot-5-pro.md`: Aktives Produkt kommt in keiner Vergleichsseite vor.
- **PRODUCT_NOT_COVERED** – `src/content/products/neakasa-m1-plus.md`: Aktives Produkt kommt in keiner Vergleichsseite vor.
