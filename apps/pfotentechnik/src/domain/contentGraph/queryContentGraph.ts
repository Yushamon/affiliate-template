...
export type ContentGraphQuery = {
  types?: ContentGraphNode['type'][];
  edgeTypes?: ContentGraphEdgeType[];
  limit?: number;
  minScore?: number;
};

export const queryContentGraph = (
  graph: ContentGraphData,
  sourceId: string,
  query: ContentGraphQuery = {}
) => {
  ...
  return graph.edges
    ...
    .sort(
      (a, b) =>
        Number(b.edge.explicit) - Number(a.edge.explicit) ||
...
export const getGraphSections = (graph: ContentGraphData, sourceId: string) => ({
  guides: queryContentGraph(graph, sourceId, {
    types: ['page'],
    edgeTypes: ['parent', 'child', 'related', 'cluster', 'semantic'],
    minScore: 20,
    limit: 4
  }),
  comparisons: queryContentGraph(graph, sourceId, {
    types: ['comparison'],
    edgeTypes: ['comparison', 'cluster', 'semantic'],
    minScore: 25,
    limit: 3
  }),
  products: queryContentGraph(graph, sourceId, {
    types: ['product'],
    edgeTypes: ['product', 'cluster', 'semantic'],
    minScore: 25,
    limit: 4
  }),
  manufacturers: queryContentGraph(graph, sourceId, {
    types: ['manufacturer'],
    edgeTypes: ['manufacturer', 'semantic'],
    minScore: 30,
    limit: 3
  })
});
