...
const PATCH_NAME = "pfotentechnik-mobile-surface-product-cta-8.1.2";
const PATCH_MARKER = "PfotenTechnik Mobile Surface + Product CTA Fix 8.1.2";
...
function patchPremiumRenderer(source) {
  if (
    ...
    source.includes("PfotenTechnik Mobile Surface + Product CTA Fix 8.1.1")
  ) {
...
function patchAlternatives(source) {
  if (
    ...
    source.includes("PfotenTechnik generic product alternatives 8.1.2")
  ) {
    return {
      content: source,
      changed: false,
  ...
  const replacement = `  /* PfotenTechnik generic product alternatives 8.1.2
...
function patchComparisonMethodology(source) {
  ...
  /*
   * Astro entfernt reinen Zeilenumbruch zwischen Inline-Element und Text.
   * Das explizite {" "} wird deshalb direkt nach den drei bekannten
   * strong-Elementen eingefügt. Diese Methode ist unabhängig von LF/CRLF,
   * Einrückung und Zeilenumbruchformat.
   */
  const labels = [
    "Eignung vor Funktionsmenge",
    "Nachteile bleiben sichtbar",
    "Entscheidung bleibt nachvollziehbar"
  ];
  ...
  for (const label of labels) {
    ...
    if (occurrences !== 1) {
      fail(
        `ComparisonMethodology: "${label}" wurde ${occurrences} Mal gefunden; erwartet wurde genau 1.`
      );
    }

    const index = content.indexOf(needle);
    const insertAt = index + needle.length;
    const following = content.slice(insertAt);

    const alreadyHasExplicitSpace =
      /^\s*\{\s*["']\s["']\s*\}/.test(following);

    if (!alreadyHasExplicitSpace) {
      content =
  ...
  for (const label of labels) {
    const needle = `<strong>${label}:</strong>`;
    const index = content.indexOf(needle);
    const following = content.slice(index + needle.length);

    if (!/^\s*\{\s*["']\s["']\s*\}/.test(following)) {
      fail(
        `ComparisonMethodology: Explizites Leerzeichen nach "${label}" fehlt weiterhin.`
      );
    }
  }
