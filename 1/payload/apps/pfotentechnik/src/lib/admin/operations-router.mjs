...
export async function handleOperationsRoute({request,response,requestUrl,origin,json,assertJsonRequest,readJsonBody}){
  ...
  try{
    ...
    if(match){const [,id,action]=match;if(request.method==="GET"&&!action){json(response,200,await getMediaJob(id),origin);return true}if(request.method==="GET"&&action==="file"){const file=await readJobFile(id,requestUrl.searchParams.get("name")||"");response.writeHead(200,{"content-type":file.contentType,"cache-control":"no-store","x-content-type-options":"nosniff",...(origin?{"access-control-allow-origin":origin,vary:"Origin"}:{})});response.end(file.buffer);return true}if(request.method==="POST"){assertJsonRequest(request);const body=await readJsonBody(request,22_000_000);if(action==="upload")json(response,200,await uploadMediaVariant(id,body),origin);else if(action==="build")json(response,200,await buildMediaJob(id),origin);else if(action==="approve")json(response,200,await approveMediaJob(id),origin);else return false;return true}}
    return false;
  }catch(error){json(response,errorStatus(error),{error:{code:"OPERATIONS_ACTION_FAILED",message:error instanceof Error?error.message:String(error)}},origin);return true}
}
