...
const PATCH_NAME = "pfotentechnik-mobile-surface-product-cta-8.1.0";
const PATCH_MARKER = "PfotenTechnik Mobile Surface + Product CTA Fix 8.1.0";
...
function patchProductRenderer(source) {
  ...
  if (!content.includes("const secondaryBuyboxCta =")) {
    const anchor = `  .filter((item: any) => item.title)
  .slice(0, 3);

const scoreLabel =`;

    const insertion = `  .filter((item: any) => item.title)
  .slice(0, 3);
  ...
  alternatives.length > 0
    ...
    : firstComparisonHref
      ...
      : categoryHref
        ? {
            href: categoryHref,
            label: category ? \`Mehr \${category} vergleichen\` : "Weitere Modelle ansehen"
          }
        : null;
...
const scoreLabel =`;
    ...
    content = replaceExactlyOnce(
      ...
      anchor,
      insertion,
      "ProductRenderer: sekundäre CTA-Logik einfügen"
    );
    notes.push("Sekundärer CTA erhält immer ein echtes Ziel");
  }

  const oldSecondary = /[ \t]*<a class="native-buybox__secondary" href="#alternativen">\s*Alternativen vergleichen\s*<\/a>/;
  ...
  if (oldSecondary.test(content)) {
    ...
    fail("ProductRenderer: sekundärer Buybox-CTA wurde nicht erkannt.");
  }
  ...
  const statusConst =
    /\nconst testStatus = asText\(\s*source\.testStatus \?\?\s*trust\.assessmentType \?\?\s*trust\.testStatus,\s*"Redaktionelle Einordnung"\s*\);\n/s;
...
function patchAlternatives(source) {
  if (source.includes("PfotenTechnik generic product alternatives 8.1.0")) {
    return {
      content: source,
      changed: false,
  ...
  const oldEnding = `  return [];
};`;

  const replacement = `  /* PfotenTechnik generic product alternatives 8.1.0
