...
def insert_nested_line(frontmatter: str, parent: str, line: str) -> str:
    ...
    for index, current in enumerate(lines):
        ...
        # Block-Mapping: contentPlatform:\n  version: 2
        if not remainder:
            lines.insert(index + 1, f"  {line}")
            return "\n".join(lines)

        # Inline-Mapping: contentPlatform: { version: 2, cluster: gps }
        if remainder.startswith("{") and remainder.endswith("}"):
            inner = remainder[1:-1].strip()
            key, value = line.split(":", 1)
            additions = [part.strip() for part in inner.split(",") if part.strip()]
            if any(part.split(":", 1)[0].strip() == key.strip() for part in additions if ":" in part):
                return frontmatter
            additions.append(f"{key.strip()}: {value.strip()}")
            lines[index] = f"{parent}: {{ " + ", ".join(additions) + " }"
            return "\n".join(lines)

        # Unbekannte einzeilige Form nicht riskant verändern.
        return frontmatter
...
backup = root / f".money-page-metadata-migrator-4.4.2-backup-{stamp}"
...
print("Money Page Metadata Migrator 4.4.2 erfolgreich abgeschlossen.")
