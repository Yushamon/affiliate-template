...
def main():
    ...
    try:
        ...
        p=subprocess.run(['npm','run','build:pfotentechnik'],cwd=root)
        if p.returncode: raise RuntimeError('Build fehlgeschlagen.')
    except Exception:
        for f in files:
            b=backup/f.relative_to(root)
            if b.exists(): shutil.copy2(b,f)
        print('\nPatch fehlgeschlagen; alle Dateien wurden zurückgesetzt.',file=sys.stderr); raise
